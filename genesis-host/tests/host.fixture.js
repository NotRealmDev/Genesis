(()=>{
  ...
  function signal(event,payload){parent.postMessage({type:'signal',event,payload},location.origin);return Promise.resolve(true)}
  window.genesisHost={
    async getConfig(){return {hostKey:'synthetic-host-key-not-a-production-credential',autoLaunch:false,browserRunning:true}},
    async connectSignal(){parent.postMessage({type:'host-ready'},location.origin)},sendSignal:signal,
    onSignal(fn){listener=fn},onStatus(){},async launchGeForce(){return {ok:true}},async captureReady(){return {found:true}},
    focusGeForce(){},sendInput(payload){parent.postMessage({type:'control',payload},location.origin)},copyKey(){},setAutoLaunch(){}
  };
  ...
  window.addEventListener('message',event=>{if(event.source===parent&&event.origin===location.origin&&event.data?.type==='signal')listener?.(event.data)});
})();
